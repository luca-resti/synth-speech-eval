...
def sq_ast_fw(config_file):
    ...
    config_sq = config_file["sq_ast"]

    current_time = datetime.now().strftime("%Y%m%d_%H%M%S")

    output_dir = config_sq["output_dir"]
    dims = ALL_DIMS
    bs = int(1)
    num_workers = int(0)

    if str(config_sq["path"]).endswith('.wav'): 
        wav_path = config_sq["path"]
        data_dir = None
    else: 
        data_dir = config_sq["path"]
        wav_path = None
    ...
    if config_sq["device"] == "gpu" and torch.cuda.is_available():
        device = "cuda"
    else:
        device = "cpu"
    ...
    for dim in dims:  # dims comes from argparse
        model = ASTXL()
        state = torch.load(f"models/weights/{dim}.pth", map_location=torch.device(device), weights_only=True)
    ...
    for dim_index in range(len(dims)):
        ...
        for b, (index, batch_features) in enumerate(dl):
            ...
            predictions[index, dim_index] = pred # store prediction for this SQ dimension

            attention_flow = tensor_attention_flow(attentions[0][0].cpu().detach())
            attention_flows[index, dim_index, :, :] = attention_flow.reshape(12, 101) # store downscaled attention flow    
